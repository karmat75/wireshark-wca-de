...
PCAP="${1:-pcaps/generated/challenge-005-security-basics-triage.pcapng}"

if [ ! -f "$PCAP" ]; then
  echo "ERROR: PCAP not found: $PCAP" >&2
  echo "Usage: bash challenges/challenge-005-security-basics-triage/check_tshark.sh <pcap-file>" >&2
  exit 2
fi
