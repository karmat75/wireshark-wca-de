...
PCAP="${1:-pcaps/generated/challenge-007-discards-retransmission-correlation.pcapng}"

if [ ! -f "$PCAP" ]; then
  echo "ERROR: PCAP not found: $PCAP" >&2
  echo "Usage: bash challenges/challenge-007-discards-retransmission-correlation/check_tshark.sh <pcap-file>" >&2
  exit 2
fi
...
check_count "TCP packets with lab web server" "tcp and ip.addr == 172.28.60.10" 50 || failures=$((failures + 1))
check_count "HTTP packets" "http and ip.addr == 172.28.60.10" 1 || failures=$((failures + 1))
check_count "HTTP bigfile request" "http.request.uri contains \"bigfile.bin\"" 1 || failures=$((failures + 1))
check_count "TCP analysis flags" "tcp.analysis.flags" 1 || failures=$((failures + 1))
check_count "Retransmission or fast retransmission" "tcp.analysis.retransmission or tcp.analysis.fast_retransmission" 1 || failures=$((failures + 1))

info_count "Duplicate ACKs" "tcp.analysis.duplicate_ack"
info_count "Fast retransmissions" "tcp.analysis.fast_retransmission"
...
tshark -r "$PCAP" \
  ...
  -e http.request.uri 2>/dev/null || true
...
tshark -r "$PCAP" \
  ...
  -e ip.dst \
  -e tcp.dstport \
  -e _ws.col.Info 2>/dev/null | head -50 || true

echo
if [ "$failures" -eq 0 ]; then
  echo "Result: basic technical checks passed."
  echo "Now compare your monitoring correlation with solution.md."
else
