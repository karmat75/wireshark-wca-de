...
PCAP="${1:-pcaps/generated/lab-basic-030-tcp-handshake-reset.pcapng}"

if [ ! -f "$PCAP" ]; then
  echo "ERROR: PCAP not found: $PCAP" >&2
  echo "Usage: bash labs/basic/lab-basic-030-tcp-handshake-reset/check_tshark.sh <pcap-file>" >&2
  exit 2
fi
...
check_count "SYN without ACK" "tcp.flags.syn == 1 and tcp.flags.ack == 0" 2 || failures=$((failures + 1))
check_count "SYN/ACK packets" "tcp.flags.syn == 1 and tcp.flags.ack == 1" 1 || failures=$((failures + 1))
check_count "HTTP packets" "http" 1 || failures=$((failures + 1))
check_count "HTTP response from web-ok" "ip.addr == 172.28.50.10 and http.response" 1 || failures=$((failures + 1))
check_count "Traffic to closed port 81" "tcp.port == 81" 1 || failures=$((failures + 1))
