...
PCAP="${1:-pcaps/generated/lab-advanced-030-performance-analyse.pcapng}"

if [ ! -f "$PCAP" ]; then
  echo "ERROR: PCAP not found: $PCAP" >&2
  echo "Usage: bash labs/advanced/lab-advanced-030-performance-analyse/check_tshark.sh <pcap-file>" >&2
  exit 2
fi
...
if [ "$failures" -eq 0 ]; then
  ...
  echo "Now compare your timing analysis with solution.md."
else
